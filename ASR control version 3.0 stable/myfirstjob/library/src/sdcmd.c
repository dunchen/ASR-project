...
//RESET:
	...
	SPI_SendByte(tmp);
#else
    if (cmd!=8) {SPI_SendByte(0x95);} /* CRC校验码,只用于第1个命令 CRC,only used for the first command */
      else
			{SPI_SendByte(0x87);}
#endif 
    ...
    {												 /* Wait for a response,a response is a start bit(zero) */ 
        tmp = SPI_RecByte();
        i++;
    }while (((tmp & 0x80) != 0) && (tmp!=0xE5) && (i < SD_CMD_TIMEOUT));
		
		//
    //if ((cmd==0) && (tmp!=0x00)) {goto RESET;}
    //
    ...
    for (i = rlen - 1; i >= 0; i--)
    {
		...
		//while (tmp!=0xFF) {tmp=SPI_RecByte();};
...
INT8U SD_ReadBlockData(INT32U len, INT8U *recbuf)
{
  ...
  timeout=1000;
	...
	if ((tmp != 0xFE) && (tmp!=0xFC))					/* 块读开始令牌错误 read start block token is error */
	{
		SPI_SendByte(0xFF);
		SPI_CS_Deassert();
...
INT8U SD_WriteBlockData(INT8U bmulti, INT32U len, INT8U *sendbuf)
{
	...
	SPI_SendByte(0x95);
	SPI_SendByte(0x95); 						/* 发送CRC16校验码 send CRC16 check code */
